...
def style_loss(CNN_structure, const_layers, var_layers, content_segs, style_segs, weight):
    ...
    for layer_name in CNN_structure:
        ...
        # if layer_index >= len(var_layers):
        #     continue

        # positions = [i for i, letter in enumerate(var_layers[layer_index].name) if letter == '/']
        ...
        # if layer_name == var_layers[layer_index].name[positions[1] + 1:positions[2]]:
            ...
            for content_seg, style_seg in zip(content_segs, style_segs):
                ...
                # if style_seg.shape[1] != const_layer.shape[1]:
                #     style_seg = style_seg[:,:const_layer.shape[1]:,:]

                # if style_seg.shape[2] != const_layer.shape[2]:
                #     style_seg = style_seg[:,:,:const_layer.shape[2]:]

                # if content_seg.shape[1] != const_layer.shape[1]:
                #     content_seg = content_seg[:,:const_layer.shape[1]:,:]

                # if content_seg.shape[2] != const_layer.shape[2]:
                #     content_seg = content_seg[:,:,:const_layer.shape[2]:]
...
def stylize(args, Matting, vgg_model):
    ...
    with tf.name_scope("constant"):
        ...
        # vgg_const = vgg_model
        # # print(vgg_const.summary())

        # # content_image = tf.keras.applications.vgg19.preprocess_input(content_image)

        # style_layers = ["conv1_1", "conv2_1", "conv3_1", "conv4_1", "conv5_1"]
        # style_layers_const_inter = Model(inputs=vgg_const.input, outputs = [vgg_const.get_layer(layer_name).output for layer_name in style_layers])

        # style_layers_const = style_layers_const_inter(content_image)

        # content_layer_const_inter = Model(inputs=vgg_const.input, outputs = vgg_const.get_layer("conv4_2").output)
    ...
    with tf.name_scope("variable"):
        ...
        # style_layers_var_inter = Model(inputs=vgg_var.input, outputs = [vgg_var.get_layer(layer_name).output for layer_name in style_layers_var_list])
        # style_layers_var = style_layers_var_inter(input_image)

        # content_layer_var_inter = Model(inputs=vgg_var.input, outputs = vgg_var.get_layer("conv4_2").output)
        # content_layer_var = content_layer_var_inter(input_image)

        # input_image = tf.Variable(input_image)


    # which layers we want to use? [OLD VGG]
    ...
    # print([layer.name for layer in vgg_var.layers])
    
    # NEW VGG
    # layer_structure_all = [layer.name for layer in vgg_var.layers]
...
#        optimizer = ScipyOptimizerInterface(overall_loss, method='L-BFGS-B', options={'maxiter': args.max_iter, 'disp': 0})
#        optimizer = tf.estimator.experimental.opt.ScipyOptimizerInterface(overall_loss, method='L-BFGS-B', options={'maxiter': args.max_iter, 'disp': 0})
...
#        optimizer.minimize(overall_loss, var_list=[loss_content, loss_styles_list, loss_tv, loss_affine,overall_loss, input_image_plus])
